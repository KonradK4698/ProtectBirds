...
export default class dodajGatunki extends Component {
    ...
    dodajFoto = async () => {
        ...
        let ref = firebase.storage().ref('ochrona/').child(name);
        ref.put(blob).then(() => {
            this.setState({ isLoadingPhoto: false })
            Alert.alert("Grafika została dodana");
            firebase.storage().ref('ochrona/').child(name).getDownloadURL().then(
    ...
    dodajDane = () => {
        var myRef = firebase.database().ref('/protection');
        // var key = myRef.key;
        ...
        return (
            <ScrollView contentContainerStyle={{ justifyContent: 'center', alignItems: 'center', padding: 10, }}>
                <Text style={styles.inputLabel}>
                    Wybierz grafikę prezentujące typ ochrony:
                </Text>
                ...
                <Text style={styles.inputLabel}>
                    Podaj tytuł opisujący typ ochrony
                </Text>
                ...
                <Text style={styles.inputLabel}>
                    Wprowadź opis dotyczący typu ochrony
                </Text>
